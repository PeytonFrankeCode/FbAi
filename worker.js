...
// most-visited kind of page on the site and this would otherwise put an extra
// asset fetch in front of every one of them. The file is a few hundred bytes
// and changes only when a build merges another pair of spellings.
//
// A failure to load is cached as an empty map, not retried per request. If the
// file is missing, the right behaviour is to serve the pages normally rather
// than to hammer the asset binding on every hit.
let _playerRedirects = null;
async function playerRedirect(env, slug) {
  if (!slug) return null;
  if (_playerRedirects === null) {
    _playerRedirects = {};
    try {
      if (env.ASSETS) {
        const resp = await env.ASSETS.fetch(new Request('https://assets.local/data/players/redirects.json'));
        // A 200 is not proof of existence here either — single-page-application
        // fallback serves index.html for a missing asset, and parsing that as
        // JSON throws. Check the type before trusting it.
        const ct = resp && resp.headers.get('content-type') || '';
        if (resp && resp.ok && /json/i.test(ct)) _playerRedirects = await resp.json();
      }
    } catch (err) {
      console.error('player redirects unavailable:', err && err.message);
    }
  }
  const to = _playerRedirects[slug];
  // Never redirect a slug onto itself; a self-referential entry would be an
  // infinite loop served as a 301.
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      //
      // Twelve players had two pages each because two checklists spelled the
      // name differently — "Ja’Marr Chase" with a curly apostrophe in one and
      // "Ja'Marr Chase" with a straight one in another, "D.K. Metcalf" against
      // "DK Metcalf". The build now folds them, which removes the smaller URL.
      //
      // Without this, that removal is invisible and wrong: not_found_handling
      // is single-page-application, so /players/cj-stroud-2/ would return the
      // SPA shell with a 200. A crawler sees a live page, keeps it indexed, and
      // the duplicate never actually goes away. /players/cj-stroud-2/ is in the
      // current sitemap, so this is not hypothetical.
      //
      // The map is generated by build-landing-pages.js from the merge itself
      // rather than typed here, so it cannot fall out of step with the pages.
      if (url.pathname.startsWith('/players/')) {
        const slug = url.pathname.split('/')[2] || '';
        const to = await playerRedirect(env, slug);
        if (to) {
          url.pathname = `/players/${to}/`;
  ...
  async scheduled(event, env, ctx) {
    ...
    ctx.waitUntil((async () => {
      try {
        ...
        // Persist the request tally, for the same reason and at the same cost:
        // one KV write a tick. An unflushed counter reports nothing, and a
        // traffic report that is always empty is worse than none, because it
        // reads as "no traffic" rather than as "not wired up".
        if (typeof flushTraffic === 'function') {
          await flushTraffic().catch(err => console.error('[Cron] traffic flush failed:', err && err.message || err));
        }
