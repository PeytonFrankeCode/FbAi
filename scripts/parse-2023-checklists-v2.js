...
// Two-part source. Concatenated before parsing because a product's blocks can
// span the split, and the parser works on one continuous line array.
const ROOT = path.join(__dirname, '..');
const DOCX_PATHS = [
  path.join(ROOT, '2023 Checklists - Part 1.docx'),
  path.join(ROOT, '2023 Checklists - Part 2.docx'),
];
...
async function main() {
  ...
  for (const p of DOCX_PATHS) {
    ...
    console.log(`Loaded ${r.value.length} chars from ${path.basename(p)}`);
    text += r.value + '\n';
  }
  ...
  // Full replacement: every existing 2023 product file goes, then we write
  // only what this source produced. Without the delete, products the old
  // source had and this one doesn't would linger as orphans — present on
  // disk, absent from the index.
  const keep = new Set(products.map(p => `${p.id}.json`));
  let removed = 0;
  for (const f of fs.readdirSync(CHECKLISTS_DIR)) {
    if (!/^2023-.*\.json$/.test(f)) continue;
    if (keep.has(f)) continue;
    fs.unlinkSync(path.join(CHECKLISTS_DIR, f));
    console.log(`  removed stale ${f}`);
    removed++;
  }
  if (removed) console.log(`Removed ${removed} 2023 product file(s) not in the new source`);
