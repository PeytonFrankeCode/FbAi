...
// Resolves the checklist for this query, classifies every result against it,
...
// Our own "Continue with Google" button is in the slot from the start. Where
// Google's script loads, its official button replaces it; where the script is
// slow or blocked (a weak connection, a content or tracker blocker), ours
// stays, and signs in through Google's redirect page instead
// (/api/auth/google/start), which needs no script on this page. Before, a
// script that never arrived left the panel with an "or" and nothing above it.
...
function _googleRedirectSignIn() {
  location.href = '/api/auth/google/start?return=' + encodeURIComponent(location.pathname + location.search);
}
function mountGoogleSignIn(clientId, slot) {
  if (!clientId || !slot) return;
  slot.classList.remove('hidden');
  slot.innerHTML = `<button type="button" class="social-auth-btn social-auth-google" onclick="_googleRedirectSignIn()">${GOOGLE_G_SVG}<span>Continue with Google</span></button>`;
  const render = () => {
    try {
      ...
      // If Google's button has not drawn (a blocked frame, an origin Google
      // refuses), put ours back rather than leave the gap.
      setTimeout(() => { if (!slot.childElementCount) slot.innerHTML = ours; }, 2500);
    } catch (err) {
...
async function loadMarketIndex() {
  ...
  if (!data || !data.available) {
    ...
    if (shortHistory) {
      ...
      title = 'Coming soon for this period';
      text = `This view compares ${_mkDays} days against the ${_mkDays} before them. Try a shorter period.`;
    } else if (thin) {
      title = 'Not enough trading to chart';
      text = `The index follows cards that sell again and again`
           + `${_mkPlayer ? `, and ${who}'s cards haven't traded that often over this period` : ''}.`
           + ` Try a longer period.`;
      ...
      title = 'Market data is refreshing';
...
function _mkRenderChart(data) {
  ...
  if (series.length < 2) {
    canvas.classList.add('hidden');
    if (empty) { empty.textContent = 'Not enough trading yet to draw a trend.'; empty.classList.remove('hidden'); }
    return;
  }
  ...
  _mkChart = new Chart(canvas.getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        ...
        // Every point is drawn the same way; how a thin day was filled is the
        // index's business, not something the chart presents to the reader.
...
<body>
  ...
  frame.style.cssText = 'position:fixed;width:0;height:0;border:0;right:0;bottom:0;opacity:0;';
  frame.onload = () => {
    try {
      ...
      try {
        ...
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url; a.download = 'card-huddle-collection.html'; a.click();
        setTimeout(() => URL.revokeObjectURL(url), 4000);
      } catch (_) {}
    }
...
function renderInvHistory() {
  ...
  const photos = getInvPhotos();
  const cardCell = (name, printRun, photoId) => {
    if (photoId) _invEnsurePhoto(photoId);
    const thumb = photoId && photos[photoId]
