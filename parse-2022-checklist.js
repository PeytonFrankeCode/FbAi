#!/usr/bin/env node
/**
 * Parse 2022 NFL checklist DOCX into per-product JSON files.
 *
 * Source: All 2022 Nfl Checklists.docx
 * Output: public/data/checklists/<product-id>.json plus an updated
 *         public/data/checklists/index.json entry per product.
 *
 * Same two block shapes as the 2022 doc — most products are team-split
 * (32 short blocks, set name baked into each card line, team in the
 * header) and a handful are Master Card List shape. Setting both
 * formats are detected per-product.
 */
...
const YEAR = 2022;
const DOCX_PATH = path.join(__dirname, 'All 2022 Nfl Checklists.docx');
...
// "... Chris Rodriguez Jr. /1492022 Clearly Donruss Football Checklist –
// Master Card List"). We scan with /g so every header on every line lands
// in the headers list.
const PRODUCT_HEADER_RE = /(2022\s+[A-Za-z][A-Za-z0-9 .'’&-]+?\s+Football)\s+Checklist(?:\s*[–-]\s*([^\r\n]{0,200}?))?(?=\s*$|2022\s|Here)/g;
...
async function main() {
  ...
  // Drop any existing 2022 products so re-runs are idempotent.
  index.products = index.products.filter(p => p.year !== 2022);
  for (const p of products) {
    index.products.push({
      id: p.id,
      name: p.name,
...
function parseTeamSplitProduct(productName, blocks, lines) {
  ...
  for (const block of blocks) {
    ...
    for (let i = block.start; i < block.end; i++) {
      ...
      // Skip any nested 2022 header (shouldn't happen because we already split blocks).
...
function isMasterSetHeader(line, slice, idx) {
  ...
  if (/^(Here'?s|Next Article|Cheap Wax|Ryan Cracknell|THE BECKETT|Subscribe|Shop Now|RELATED|LEAVE|Top of|Bottom of|Stay in|LATEST|NEW CHECK|1 COMMENT|Collecting|What does|Copyright|SUBJECT|Please reach|Please note|You can|Check out|View |Buy on|Refer to|Highest print|Updates to previous|Versions:|Parallels:)/i.test(line)) return false;
...
function parseMasterSet(slice, startIdx, category) {
  ...
  while (i < slice.length) {
    ...
    if (/^2022\s/i.test(cl) && /Football/i.test(cl)) break;
...
function parseMasterCardLine(line) {
  ...
  for (let i = 0; i < starts.length; i++) {
    ...
    if (cm) {
      ...
      // Team-less fallback: some products (collegiate releases, Select Draft
      // Picks) list cards as "N Player" with no comma+team. Keep the card
      // even without a team field so the dropdown still surfaces it.
      const tm = clean.match(/^(\d+)\s+(.+?)\s*$/);
      if (tm && /[A-Z][a-z]/.test(tm[2])) {
...
function deriveBrand(productName) {
  ...
  // is a no-op there. e.g. "2022 Panini Mosaic Football" -> "Mosaic".
  return productName
    .replace(/^2022\s+/, '')
    .replace(/^Panini\s+/, '')
    .replace(/\s+Football$/, '')
