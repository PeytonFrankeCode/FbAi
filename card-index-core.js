// The player resolver, with no data attached.
//
// Split from card-index.js so the Worker can reach the logic without the
// 918 KB of JSON coming with it. A Worker compiles its whole script before
// serving anything, so anything reachable from server.js is paid for on every
// cold start — which took the site down. The data is fetched from the assets
// binding instead and passed in here.
...
function createCardIndex(INDEX) {
...
module.exports = { createCardIndex, norm };
