...
function getMockData(query) {
  ...
  const prices = [249.99, 89.00, 134.50, 45.00, 312.00, 178.00, 67.50, 220.00];
  const conditions = ['Graded - PSA 10', 'Graded - PSA 9', 'Graded - BGS 9.5', 'Near Mint or Better', 'Graded - PSA 8', 'Near Mint or Better', 'Graded - PSA 9', 'Graded - PSA 10'];
  const suffixes = [
    '2020 Panini Prizm PSA 10 Gem Mint',
    '2020 Panini Prizm Silver PSA 9',
    '2021 Donruss Optic Holo Rookie RC BGS 9.5',
    '2020 Panini Prizm Red White Blue /175',
    '2022 Topps Chrome Refractor Auto #/99',
    '2021 Select Concourse RC',
    '2020 Mosaic Silver Prizm PSA 9',
    '2022 Prizm Silver Holo PSA 10',
  ];
  return {
    results: suffixes.map((suffix, i) => ({
      itemId: `demo-${i}`,
      title: `${query} - ${suffix}`,
      price: String(prices[i]),
      currency: 'USD',
      soldDate: day((i + 1) * 86400000),
      imageUrl: null,
...
async function performSearch(query) {
  ...
  // Simulate a short delay so the loading spinner is visible
  ...
  priceChart = new Chart(chartCanvas, {
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          grid: { color: '#2d3748' },
          ticks: { color: '#8d99ae', font: { size: 11 }, callback: val => `$${val}` },
          beginAtZero: false
        }
      }
    }
...
const TEAM_COLORS = {
  ...
  purdy: '#AA0000', niners: '#AA0000', '49ers': '#AA0000',
...
function buildCard(item) {
  ...
  const price = item.price ? `$${parseFloat(item.price).toFixed(2)}` : 'Price N/A';
